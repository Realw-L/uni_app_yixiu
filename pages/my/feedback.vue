...
export default {
	...
	methods: {
				...
				success: res => {
					...
					wx.uploadFile({
						...
						success: res => {
							...
							console.debug(' images:' + _self.imageList);
						},
						fail: err => {
		...
		send() {
			...
			var reqData = {
				score: _self.sendDate.score,
				description: _self.sendDate.content,
				category: 'sys',
				contact: _self.sendDate.contact,
				images: _self.ids.join(',')
			};
			//api: message/feedback_add
			util.POST('message/feedback_add', reqData, function(data) {
				console.info('handle:' + JSON.stringify(data));
				if (data.status == 1) {
					uni.showToast({ title: '提交成功， 谢谢你的宝贵意见和建议！', icon: 'none' });
					//switch to home
					setTimeout(function() {
						uni.switchTab({
